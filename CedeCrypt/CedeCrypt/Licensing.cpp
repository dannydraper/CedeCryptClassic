...
void Licensing::SetDiagnostics (Diagnostics *pdiag)
{
	m_pdiag = pdiag;
}
...
bool Licensing::PrepareDLLPath ()
{
	...
	strcat_s (m_szDLLPath, SIZE_STRING, "\\CedeSoft\\CedeCrypt\\CCrypt.dll");
...
void Licensing::SaveMachineKey ()
{
	...
	memFile.Write (&iencsize, 46144, sizeof (int));

	// Write the encoded key to the file
	memFile.Write (memEncoded.GetBuffer (), 46152, memEncoded.GetSize ());
...
bool Licensing::LoadMachineKey (char *szOutKey)
{
	...
	if (memFile.ReadFromFile (m_szDLLPath) == true) {
		...
		int iencsize = 0;
		memcpy (&iencsize, (BYTE *) memFile.GetBuffer ()+46144, sizeof (int));

		// Now read the encoded machine key
		MemoryBuffer memEncoded;
		memEncoded.SetSize (iencsize);
		memEncoded.Write ((BYTE *) memFile.GetBuffer ()+46152, 0, iencsize);
...
void Licensing::SaveUserKey (char *szUserKey)
{
	...
	// Write the size to the file
	int iencsize = memEncoded.GetSize ();
	memFile.Write (&iencsize, 46912, sizeof (int));

	// Write the encoded key to the file
	memFile.Write (memEncoded.GetBuffer (), 46944, memEncoded.GetSize ());
	memFile.SaveToFile (m_szDLLPath);
}
...
bool Licensing::LoadUserKey (char *szOutkey)
{
	...
	if (memFile.ReadFromFile (m_szDLLPath) == true) {
		// Get the size of the encoded key from the file
		int iencsize = 0;
		memcpy (&iencsize, (BYTE *) memFile.GetBuffer ()+46912, sizeof (int));

		// Now read the encoded user key
		MemoryBuffer memEncoded;
		memEncoded.SetSize (iencsize);
		memEncoded.Write ((BYTE *) memFile.GetBuffer ()+46944, 0, iencsize);
...
void Licensing::OutputInt (LPCSTR lpszText, int iValue)
{
...
void Licensing::OutputText (LPCSTR lpszText)
{
	if (m_bUseDiagnostics == true) {
...
void Licensing::OutputText (LPCSTR lpszName, LPCSTR lpszValue)
{
	if (m_bUseDiagnostics == true) {
