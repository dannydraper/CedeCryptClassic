...
void UIWindow::CreateAppWindow (LPCSTR lpszClassName, int xPos, int yPos, int Width, int Height, bool bChild)
{
	...
	if (m_bStyleset == true) {
		if (m_iSelectedstyle == FS_STYLESTANDARD) {
			if (bChild == true) {
				g_hWnd = CreateWindowEx (WS_EX_WINDOWEDGE, lpszClassName, g_lpszCaption, WS_MINIMIZEBOX| WS_CAPTION | WS_POPUPWINDOW, xPos, yPos, Width, Height, NULL, NULL, GetModuleHandle (NULL), (void *) this);
			} else {
				g_hWnd = CreateWindowEx (WS_EX_WINDOWEDGE, lpszClassName, g_lpszCaption, WS_MINIMIZEBOX| WS_CAPTION | WS_POPUPWINDOW, xPos, yPos, Width, Height, NULL, NULL, GetModuleHandle (NULL), (void *) this);
			}
		}
...
HWND UIWindow::CreateTextBox (int xPos, int yPos, int Width, int Height, unsigned int iControlID)
{
	...
	hwndcontrol = CreateWindow ("edit", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT , xPos, yPos, Width, Height, g_hWnd, (HMENU) iControlID, GetModuleHandle (NULL), NULL);
	SendMessage (hwndcontrol, WM_SETFONT, (WPARAM) hfDefault, MAKELPARAM (FALSE, 0));
