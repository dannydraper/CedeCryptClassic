...
MainWindow::MainWindow ()
{
	...
	m_ccryptid2 = 477771545;
	m_ccryptid3 = 477771547;
	...
	m_ldatasignaturelocation = 0;
	m_iNumpackagedfiles = 0;
}
...
{	
	...
	SetCaption (TEXT ("CedeCrypt Encrypted Package"));
	...
	if (m_bCreatewindow == true) {
		...
		CreateAppWindow ("CRYPTWindowClass", 70, 0, iwidth, iheight, false);
		
		m_uihandler.SetWindowProperties (0, 0, 0, 0, GetSysColor (COLOR_BTNFACE));
...
void MainWindow::OnCreate (HWND hWnd)
{
	...
	//m_hwndgroup1 = CreateWindow ("button", "Decryption Only", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 12, 57, 419, 50, hWnd, (HMENU) ID_GROUPBOX1, GetModuleHandle (NULL), NULL);
	//SendMessage (m_hwndgroup1, WM_SETFONT, (WPARAM) hfDefault, MAKELPARAM (FALSE, 0));
	//ShowWindow (m_hwndgroup1, SW_HIDE);

	// The encrypt option
	//m_hwndoptencrypt = CreateWindow ("button", "Encrypt", WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON, 100, 77, 100, 16, hWnd, (HMENU) ID_OPTENCRYPT, GetModuleHandle (NULL), NULL);
	//SendMessage (m_hwndoptencrypt, WM_SETFONT, (WPARAM) hfDefault, MAKELPARAM (FALSE, 0));
	//EnableWindow (m_hwndoptencrypt, FALSE);
	//ShowWindow (m_hwndoptencrypt, SW_HIDE);

	// The decrypt option
	//m_hwndoptdecrypt = CreateWindow ("button", "Decrypt", WS_CHILD | WS_VISIBLE | BS_RADIOBUTTON, 270, 77, 100, 16, hWnd, (HMENU) ID_OPTDECRYPT, GetModuleHandle (NULL), NULL);
	//SendMessage (m_hwndoptdecrypt, WM_SETFONT, (WPARAM) hfDefault, MAKELPARAM (FALSE, 0));
	//ShowWindow (m_hwndoptdecrypt, SW_HIDE);

	//m_hwndgroup = CreateWindow ("button", "Decrypt file or folder?", WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 12+xoffset, 57+ioffset1, 419, 50, hWnd, (HMENU) ID_GROUPBOX, GetModuleHandle (NULL), NULL);
	...
	//m_chkkeepopen = CreateWindow ("button", "Delete original files after decryption", WS_CHILD | WS_VISIBLE | BS_CHECKBOX, 28+xoffset, 210+ioffset1, 240, 19, hWnd, (HMENU) ID_CHKDELETEORIG, GetModuleHandle (NULL), NULL);
	...
	// The exit button
	//m_btncancel = CreateWindow ("button", "Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 357+xoffset, 75+ioffset1, 70, 23, hWnd, (HMENU) ID_BTNCANCEL, GetModuleHandle (NULL), NULL);
	...
	//	m_hwndphase = CreateWindow ("static", "", WS_CHILD | WS_VISIBLE, 12+xoffset, 24+ioffset1, 270, 15, hWnd, (HMENU) ID_LBLPHASE, GetModuleHandle (NULL), NULL);
	//	m_hwndlbltotal = CreateWindow ("static", "", WS_CHILD | WS_VISIBLE, 12+xoffset, 49, 415, 15, hWnd, (HMENU) ID_LBLTOTAL, GetModuleHandle (NULL), NULL);
	//} else {
	m_hwndphase = CreateWindow ("static", "", WS_CHILD | WS_VISIBLE, 12+xoffset, 15+ioffset1, 270, 15, hWnd, (HMENU) ID_LBLPHASE, GetModuleHandle (NULL), NULL);
	m_hwndlbltotal = CreateWindow ("static", "", WS_CHILD | WS_VISIBLE, 12+xoffset, 63+ioffset1, 415, 15, hWnd, (HMENU) ID_LBLTOTAL, GetModuleHandle (NULL), NULL);
	//}
	...
	// The progress bar
	//if (m_bShellmode == true || m_bCommandmode == true || m_bSessionmode == true) {
		m_hwndprogress  = CreateWindowEx(0L, PROGRESS_CLASS, "", WS_CHILD | WS_VISIBLE, 12+xoffset, 30+ioffset1, 415, 22, hWnd, (HMENU) ID_PROGRESS, GetModuleHandle (NULL), NULL);
	//} else {
	...
	//if (m_bShellmode == true || m_bCommandmode == true || m_bSessionmode == true) {
		m_hwndtotalprogress  = CreateWindowEx(0L, PROGRESS_CLASS, "", WS_CHILD | WS_VISIBLE, 12+xoffset, 78+ioffset1, 415, 22, hWnd, (HMENU) ID_TOTALPROGRESS, GetModuleHandle (NULL), NULL);
	//} else {
	...
	m_header.SetBitmapProperties (0, 0, 161, 143);
	m_header.SetProperties (hWnd, CID_HEADER, 0, 0, 161, 143);
	...
	OutputStatusText ("CedeCrypt v2.3 Initialised and Ready.");
	...
	*/
	...
	if (ScanForSelfData () == true) {
		// Begin decrypting the data package
		int numfiles = 0;

		int ipointer = 0;

		ipointer = sizeof (unsigned long) *4;

		memcpy (&numfiles, (BYTE *) m_memPackage.GetBuffer ()+ipointer, sizeof (int));

		m_pdiag->OutputInt ("Number of Encrypted Files: ", numfiles);

		StartSelfDecryption ();
	}
}

bool MainWindow::ScanForSelfData ()
{
	// This function will scan this application executable for the signature which will
	...
	unsigned long lcursig1 = 0;
	unsigned long lcursig2 = 0;
	unsigned long lcursig3 = 0;
	unsigned long lcursig4 = 0;

	char szApp[SIZE_STRING];
	ZeroMemory (szApp, SIZE_STRING);

	GetModuleFileName (NULL, szApp, SIZE_STRING);

	MemoryBuffer memApp;
	if (memApp.ReadFromFile (szApp) == true) {
		
		for (b=0;b<memApp.GetSize ();b++) {
			
			if (b>(sizeof (unsigned long)*4)) {
				
				c = b-(sizeof (unsigned long)*4);

				memcpy (&lcursig1, (BYTE *) memApp.GetBuffer ()+c, sizeof (unsigned long));
				memcpy (&lcursig2, (BYTE *) memApp.GetBuffer ()+c+sizeof (unsigned long), sizeof (unsigned long));
				memcpy (&lcursig3, (BYTE *) memApp.GetBuffer ()+c+(sizeof (unsigned long)*2), sizeof (unsigned long));
				memcpy (&lcursig4, (BYTE *) memApp.GetBuffer ()+c+(sizeof (unsigned long)*3), sizeof (unsigned long));

				if (lcursig1 == lpsig1 && lcursig2 == lpsig2 && lcursig3 == lpsig3 && lcursig4 == lpsig4) {
					m_ldatasignaturelocation = c;

					m_memPackage.SetSize (memApp.GetSize ()-c);
					m_memPackage.Write ((BYTE *) memApp.GetBuffer ()+c, 0, memApp.GetSize ()-c);
					memApp.Clear ();
		...
		MessageBox (NULL, "CedeCrypt was unable to open the file for Signature scanning.", "Fatal Error", MB_OK);
	}
...
void MainWindow::GetPackageFileName (int iIndex, char *szDestname)
{
	// Function which looks at the attached package data, and given an index returns the file
	// name at that index, and copies the file name to the given char pointer.

	// Package format:
	// SIG1 | SIG2 | SIG3 | SIG4 | NUMFILES | NAMELEN | NAME | FILELEN | FILE
	// ulong| ulong| ulong| ulong| int      | int     | char | ulong   | byte

	// First ensure that the index has exceeded the number of files in the package
	int numfiles = 0;
	int ipointer = 0;
	int b = 0;
	...
	ipointer = sizeof (unsigned long) *4; // Skip over the signature
	memcpy (&numfiles, (BYTE *) m_memPackage.GetBuffer ()+ipointer, sizeof (int)); // Get the number of files
	ipointer+=sizeof (int);

	if (iIndex >=0 && iIndex < numfiles) {
		
		for (b=0;b<numfiles;b++) {
			
			memcpy (&namelen, (BYTE *) m_memPackage.GetBuffer ()+ipointer, sizeof (int));
			ipointer+=sizeof (int);

			ZeroMemory (szName, SIZE_STRING);
			memcpy (szName, (char *) m_memPackage.GetBuffer ()+ipointer, namelen);
			ipointer+=namelen;

			memcpy (&filelen, (BYTE *) m_memPackage.GetBuffer ()+ipointer, sizeof (unsigned long));
			...
			if (iIndex == b) {
				ZeroMemory (szDestname, SIZE_STRING);
				strcpy_s (szDestname, SIZE_STRING, szName);
				return;
			}
		}
	}
...
void MainWindow::GetPackageFile (int iIndex, MemoryBuffer *memDest)
{
	// Function which looks at the attached package data, and given an index returns the file
	// at that index, and copies the file to the given Memory Buffer.
	...
	if (iIndex >=0 && iIndex < numfiles) {
		...
		for (b=0;b<numfiles;b++) {
			...
			// Skip over the file name itself
			ipointer+=namelen;
			...
			if (iIndex == b) {
				memDest->Clear ();
				memDest->SetSize (filelen);
				memDest->Write ((BYTE *) m_memPackage.GetBuffer ()+ipointer, 0, filelen);
				return;
			}
	...
	if (m_filemode == MODE_FILE) {	
		if (strlen (m_szInputfile) > 0) {
			if (strcmp (m_szInputfile, "<No Source Selected>") == 0) {
				if (m_filemode == MODE_FILE) {
					MessageBox (m_hwnd, "Please select a source file to decrypt.", "CedeCrypt", MB_OK | MB_ICONEXCLAMATION);
					return;
				}
				if (m_filemode == MODE_FOLDER) {
					MessageBox (m_hwnd, "Please select a source folder to decrypt.", "CedeCrypt", MB_OK | MB_ICONEXCLAMATION);
					return;
				}				
			}
		} else {
...
void MainWindow::DoEncryption ()
{
	...
	if (m_crypt.CreateKeyHive (m_szPassword) == true) {
		...
		// Retrieved the encrypted file from the package
		GetPackageFile (m_iFilepointer, &m_fileinput);
...
void MainWindow::GetFileNameOnly (char *szInpath, char *szOutpath)
{
	...
	strcpy_s (szOutpath, SIZE_STRING, szInpath+seploc);
}
...
void MainWindow::SaveEncryptedFile ()
{
	...
	if (m_filemode == MODE_FOLDER) {
		...
		if (m_appmode == MODE_DECRYPT) {
			...
			if (m_inputchecksum != m_checksum) {
				...
				if (m_iFilepointer == m_iNumpackagedfiles-1) {
				...
				if (m_bCommandmode == true) {
					...
					// NOw construct the path to save the file
					char szModpath[SIZE_STRING];
					ZeroMemory (szModpath, SIZE_STRING);

					GetModuleFileName (NULL, szModpath, SIZE_STRING);

					char szPath[SIZE_STRING];
					ZeroMemory (szPath, SIZE_STRING);
					GetPathOnly (szModpath, szPath);

					char szFile[SIZE_STRING];
					ZeroMemory (szFile, SIZE_STRING);
					GetFileNameOnly (m_szInputfile, szFile);

					char szFinal[SIZE_STRING];
					ZeroMemory (szFinal, SIZE_STRING);
					strcpy_s (szFinal, SIZE_STRING, szPath);
					strcat_s (szFinal, SIZE_STRING, "DecryptedFiles\\");
					_mkdir (szFinal);
					strcat_s (szFinal, SIZE_STRING, szFile);

					char szProposed[SIZE_STRING];
					ZeroMemory (szProposed, SIZE_STRING);
					m_filehandler.CheckDuplicatedFile (szFinal, szProposed);

					m_fileoutput.SaveToFile (szProposed);
				}
				...
				if (m_iFilepointer == m_iNumpackagedfiles-1) {
					if (m_bCommandmode == false) {
						if (m_bSessionmode == false) {
							...
							GetModuleFileName (NULL, szmodpath, SIZE_STRING);
							GetPathOnly (szmodpath, szPath);
							strcat_s (szPath, SIZE_STRING, "DecryptedFiles\\");

							sprintf_s (szmessage, SIZE_STRING, "%d File(s) decrypted successfully. The decrypted files have been saved to %s.", m_iNumpackagedfiles, szPath);

							MessageBox (m_hwnd, szmessage, "CedeCrypt Decryption", MB_OK | MB_ICONINFORMATION);
						}
					}					
	...
	if (m_filemode == MODE_FILE) {
		...
		if (m_appmode == MODE_ENCRYPT) {
			...
			m_fileoutputex.SetSize (m_fileoutput.GetSize ()+(sizeof (unsigned int)*2)+m_memEncrypted.GetSize ()+m_memCanvasEncrypted.GetSize () + sizeof(int)+ strlen (m_szDistributionname) + sizeof (int) + sizeof (int));
		...
		if (m_filemode == MODE_FOLDER) {
			if (m_iFilepointer < m_iNumpackagedfiles-1) {
				...
				if (m_appmode == MODE_DECRYPT) {
					...
					GetPackageFileName (m_iFilepointer, m_szInputfile);
...
void MainWindow::StartSelfDecryption ()
{
	// Begin decrypting the data package
	int numfiles = 0;
	...
	SendMessage(m_hwndtotalprogress, PBM_SETRANGE32, 0, m_iNumpackagedfiles-1);
	SendMessage (m_hwndtotalprogress, PBM_SETSTEP, 1, 0L);

	char szInfo[SIZE_STRING];
	ZeroMemory (szInfo, SIZE_STRING);
	sprintf_s (szInfo, SIZE_STRING, "This self-decrypting package contains %d encrypted files.\n\nPlease enter the password to decrypt the contents of this package.", numfiles);

	m_passwindow.Initialise (m_hwnd, 0);
	m_passwindow.SetEncryptMode (false);
	m_passwindow.SetSessionMode (false, false);
	m_passwindow.SetInfoLabel (szInfo);
	m_passwindow.Show ();
}
...
void MainWindow::OnCommand (HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	switch (wParam) {
		...
		case IDM_TOOLS_OPTIONS:
		{
			...
			ZeroMemory (szname, SIZE_STRING);
			GetModuleFileName (NULL, szname, SIZE_STRING);

			char szpath[SIZE_STRING];
			ZeroMemory (szpath, SIZE_STRING);

			GetPathOnly (szname, szpath);

			char szfile[SIZE_STRING];
			ZeroMemory (szfile, SIZE_STRING);
			GetFileNameOnly (szname, szfile);

			m_pdiag->OutputText ("ModPath: ", szname);
			m_pdiag->OutputText ("Path: ", szpath);
			m_pdiag->OutputText ("File: ", szfile);
		...
		case IDM_FILE_TEST:
		{
			...
			char m_szApploc[SIZE_STRING];
			ZeroMemory (m_szApploc, SIZE_STRING);
			GetModuleFileName (NULL, m_szApploc, SIZE_STRING);

			m_pdiag->OutputText ("Module Path: ", m_szApploc);

			MemoryBuffer memViewer;
			memViewer.ReadFromFile (m_szApploc);
			
			MemoryBuffer memPackage;
			memPackage.ReadFromFile ("C:\\Temp\\enctest\\Package.dat");

			MemoryBuffer memCombined;
			memCombined.SetSize (memViewer.GetSize () + memPackage.GetSize ());
			memCombined.Append (memViewer.GetBuffer (), memViewer.GetSize ());
			memCombined.Append (memPackage.GetBuffer (), memPackage.GetSize ());

			memCombined.SaveToFile ("C:\\Temp\\enctest\\EncryptedPackage.exe");
	...
	switch (controlID) {
		case CID_PASSWORDOK:
		{
			...
			if (m_filemode == MODE_FOLDER) {
				...
				if (m_appmode == MODE_DECRYPT) {
					...
					// The the package file name for the current file pointer
					// which at this stage will be zero
					GetPackageFileName (m_iFilepointer, m_szInputfile);

					m_pdiag->OutputText ("Package Infile: ", m_szInputfile);

					ZeroMemory (m_szOutputfile, SIZE_STRING);

					GetPackageFileName (m_iFilepointer, m_szOutputfile);
				}				
			}
		...
		case CID_PASSWORDCANCEL:
		{
			...
			if (m_bSessionmode == true) {
				...
				MessageBox (m_hwnd, "You must provide a password to decrypt this package.", "CedeCrypt", MB_OK | MB_ICONINFORMATION);
			}
