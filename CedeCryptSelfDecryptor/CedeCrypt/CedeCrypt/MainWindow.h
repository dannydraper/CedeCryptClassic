...
class MainWindow : public UIWindow
{
	...
	private:
		...
		bool ScanForSelfData ();
		void StartSelfDecryption ();
		void GetPackageFileName (int iIndex, char *szDestname);
		void GetPackageFile (int iIndex, MemoryBuffer *memDest);
		void GetPathOnly (char *szInpath, char *szOutpath);
		void GetFileNameOnly (char *szInpath, char *szOutpath);
		...
		// File Handler - used for checking
		// and proposing new destination file
		// names
		...
		unsigned long m_ldatasignaturelocation;
		bool m_btextmode;
		...
		// Packaged Encrypted Data
		MemoryBuffer m_memPackage;
		int m_iNumpackagedfiles;
