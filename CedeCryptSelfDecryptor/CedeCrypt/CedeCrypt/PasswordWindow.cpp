...
{	
	...
	if (m_bInitialised == false) {
		m_bInitialised = true;
		CreateAppWindow (m_szClassname, 70, 0, 299, 230, true);
...
{				
	...
	m_hwndsessionlbl = CreateWindow ("static", "This self-decrypting package contains Encrypted data.\n\nPlease enter the password to decrypt the contents of this package.", WS_CHILD | WS_VISIBLE, 10, 100, 273, 67, hWnd, (HMENU) ID_LBLSESSIONINFO, GetModuleHandle (NULL), NULL);
	...
	// The ok button
	m_hwndbtnpassok = CreateWindow ("button", "Ok", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 138, 167, 70, 23, hWnd, (HMENU) ID_BTNPASSOK, GetModuleHandle (NULL), NULL);
	SendMessage (m_hwndbtnpassok, WM_SETFONT, (WPARAM) hfDefault, MAKELPARAM (FALSE, 0));

	// The cancel button
	m_hwndbtnpasscancel = CreateWindow ("button", "Cancel", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 213, 167, 70, 23, hWnd, (HMENU) ID_BTNPASSCANCEL, GetModuleHandle (NULL), NULL);
...
void PasswordWindow::SetInfoLabel (char *szText)
{
	SetDlgItemText (m_hwnd, ID_LBLSESSIONINFO, szText);
}
