...
{	
	...
	SetCaption (TEXT ("CedeCrypt File Decryption v2.43"));
	...
	if (m_bCreatewindow == true) {
		if (DoesShellCommandExist () == true) {
			m_bShellmode = true;
			CreateAppWindow ("CRYPTWindowClass", 70, 0, iwidth, 220, false);
		} else {
...
void MainWindow::OnCreate (HWND hWnd)
{
	...
	// The progress bar
	if (m_bShellmode == true || m_bCommandmode == true || m_bSessionmode == true) {
		m_hwndprogress  = CreateWindowEx(0L, PROGRESS_CLASS, "", WS_CHILD | WS_VISIBLE, 12+xoffset, 40+ioffset1, 415, 22, hWnd, (HMENU) ID_PROGRESS, GetModuleHandle (NULL), NULL);
	} else {
	...
	if (m_bShellmode == true || m_bCommandmode == true || m_bSessionmode == true) {
		m_hwndtotalprogress  = CreateWindowEx(0L, PROGRESS_CLASS, "", WS_CHILD | WS_VISIBLE, 12+xoffset, ioffset1, 415, 22, hWnd, (HMENU) ID_TOTALPROGRESS, GetModuleHandle (NULL), NULL);
	} else {
	...
	*/
	...
	m_header.SetBitmapProperties (0, 0, 161, 424);
	m_header.SetProperties (hWnd, CID_HEADER, 0, 0, 161, 424);
